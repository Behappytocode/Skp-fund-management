...
import { Wallet, History, CreditCard, CheckCircle2, AlertCircle, Plus, ChevronRight, Clock } from 'lucide-react';
...
const MemberView: React.FC<MemberViewProps> = ({ state, user, onLoanRequest }) => {
  ...
  const stats = useMemo(() => {
    ...
    return { totalDeposited, remainingDebt, activeLoan };
  }, [myDeposits, myLoans]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-slate-900">Member Portal</h1>
          <p className="text-slate-500 text-sm">Personal contribution & loan overview.</p>
        </div>
        {!stats.activeLoan && (
          <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-indigo-600 rounded-[2rem] p-6 text-white shadow-xl shadow-indigo-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-indigo-100 text-[10px] font-black uppercase tracking-widest">Contributed</p>
            <h2 className="text-2xl font-black">{formatCurrency(stats.totalDeposited)}</h2>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h2 className="text-2xl font-black text-slate-900">{formatCurrency(stats.remainingDebt)}</h2>
          </div>
        </div>
          ...
          {stats.activeLoan ? (
            <>
              ...
              <div>
                ...
                <h2 className="text-xl font-black text-slate-900">
                  {formatCurrency(stats.activeLoan.installments.find(i => i.status === 'PENDING')?.amount || 0)}
                </h2>
              </div>
            </>
          ) : (
            <>
              ...
              <div>
                ...
                <h2 className="text-xl font-black text-emerald-600 italic">No Dues</h2>
              </div>
            </>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
            ...
            <div className="max-h-[250px] overflow-y-auto">
              <table className="w-full text-left">
                <tbody className="divide-y divide-slate-50">
                  {myRequests.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase">{r.term} Months</p>
                      </td>
                  ...
                  {myRequests.length === 0 && (
                    <tr>
                      <td className="px-6 py-12 text-center text-slate-400 italic text-sm">No recent requests.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <h3 className="font-black text-slate-900 uppercase tracking-widest text-xs flex items-center gap-2">
                <History className="text-indigo-600 w-4 h-4" /> Deposits
              </h3>
            </div>
                ...
                <tbody className="divide-y divide-slate-50">
                  {myDeposits.map((d) => (
                    <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        {d.receiptImage && (
                          <button onClick={() => window.open(d.receiptImage)} className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-xl uppercase tracking-widest">View</button>
                        )}
                      </td>
                    </tr>
                  ))}
                  {myDeposits.length === 0 && (
                    <tr>
                      <td className="px-6 py-12 text-center text-slate-400 italic text-sm">No deposits logged.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-200 p-6 shadow-sm">
          <h3 className="font-black text-slate-900 uppercase tracking-widest text-xs mb-6 flex items-center gap-2">
            <CreditCard className="text-amber-500 w-4 h-4" /> Repayment Plan
          </h3>
          {stats.activeLoan ? (
            <div className="space-y-4">
              <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <div>
                  ...
                  <p className="text-lg font-black">{formatCurrency(stats.activeLoan.recoverableAmount)}</p>
                </div>
                <div className="text-right">
                  <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Grant (30%)</p>
                  <p className="text-lg font-black text-emerald-500">{formatCurrency(stats.activeLoan.waiverAmount)}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 max-h-[250px] overflow-y-auto pr-2">
                {stats.activeLoan.installments.map((inst, i) => (
            ...
            <div className="h-48 flex flex-col items-center justify-center text-slate-300">
              ...
              <p className="italic text-xs font-bold uppercase tracking-widest">Debt Free Circle</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] w-full max-w-md overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-300">
            <div className="bg-slate-900 p-6 text-white flex justify-between items-center">
              <h2 className="text-xl font-black">Request Emergency Loan</h2>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div>
                <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Requested Amount (Rs.)</label>
                <input 
                  type="number" 
                  required
              ...
              <div>
                <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Term (Up to 6 Months)</label>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-dashed border-slate-200">
                <p className="text-[10px] text-slate-500 font-bold text-center leading-relaxed">
                  * Note: Member requests are limited to 6 months installments. Loans are subject to 70% recovery and 30% grant rule upon admin approval.
                </p>
              </div>
